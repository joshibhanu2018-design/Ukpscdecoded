 ...
 * include one, so the token itself carries the (userId, expiry) and is
 * verified on each request instead of looked up.
 */
export function createSessionToken(userId: string): { token: string; expiresAt: Date } {
  const expiresAt = new Date(Date.now() + SESSION_TTL_MS);
  const payload = Buffer.from(JSON.stringify({ sub: userId, exp: expiresAt.getTime() })).toString(
    "base64url"
  );
