...
const MIN_CHARGE_PAISE = 100; // ₹1 floor so Razorpay never sees a ₹0 order
...
export async function POST(request: NextRequest) {
  ...
  const afterCode = Math.max(MIN_CHARGE_PAISE, basePaise - codeDiscountPaise);
  const creditApplied = Math.min(availableCredit, afterCode - MIN_CHARGE_PAISE);
  const finalPaise = Math.max(MIN_CHARGE_PAISE, afterCode - creditApplied);
