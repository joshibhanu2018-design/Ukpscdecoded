...
const input = "rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none focus:border-yellow-500";
...
export default function AdminMentorshipPage() {
  ...
  const bookingCard = (b: Booking) => (
    <li key={b.id} className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 text-sm">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        <button onClick={() => act({ action: "update_booking", id: b.id, mentor_notes: notes[b.id] ?? "" }, "Plan saved")} className="rounded-lg bg-yellow-500 px-3 py-1.5 text-xs font-semibold text-slate-900">
          Save plan
        </button>
        <button onClick={() => act({ action: "update_booking", id: b.id, status: "completed", mentor_notes: notes[b.id] ?? "" }, "Marked completed")} className="rounded-lg border border-green-500/40 px-3 py-1.5 text-xs text-green-300">
          Completed
        </button>
        <button onClick={() => act({ action: "update_booking", id: b.id, status: "no_show" }, "Marked no-show")} className="rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-slate-300">
          No-show
        </button>
        {b.status === "booked" && (
          <button
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-slate-900 px-4 py-8">
      <div className="mx-auto max-w-4xl">
        ...
        {msg && <p className={`mt-3 text-sm ${msg.ok ? "text-green-300" : "text-red-300"}`}>{msg.text}</p>}
        ...
        <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
          ...
          <ul className="mt-3 divide-y divide-slate-800 text-sm">
            {windows.map((w) => (
              <li key={w.id} className="flex items-center justify-between py-2 text-slate-300">
                <span>
                  {DAYS[w.weekday]} {w.start_time.slice(0, 5)}–{w.end_time.slice(0, 5)} · {w.slot_minutes}-min slots
                </span>
                <button onClick={() => act({ action: "remove_window", id: w.id }, "Removed")} className="text-xs text-red-300">
                  Remove
                </button>
              </li>
            ))}
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button onClick={() => act({ action: "add_window", ...win, weekday: Number(win.weekday), slot_minutes: Number(win.slot_minutes) }, "Window added")} className="rounded-lg bg-yellow-500 px-4 py-2 text-sm font-semibold text-slate-900">
              Add
            </button>
          </div>
        </section>
        ...
        <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button onClick={() => block.day && act({ action: "block_date", ...block }, "Day blocked")} className="rounded-lg bg-yellow-500 px-4 py-2 text-sm font-semibold text-slate-900">
              Block day
            </button>
          </div>
