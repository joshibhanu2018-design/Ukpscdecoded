  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-slate-900 px-4 py-8">
      <div className="mx-auto max-w-6xl">
        ...
        {rows.length === 0 ? (
          <p className="mt-8 text-sm text-slate-400">No mentorship students yet.</p>
        ) : (
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-slate-800/40">
                      <td className="px-3 py-2">
                        ...
                        <div className="text-[11px] text-slate-500">{m.phone ?? m.email}</div>
                      </td>
                      ...
                      <td className="px-3 py-2 text-xs">{nextByUser.has(m.id) ? fmt(nextByUser.get(m.id)!) : <span className="text-slate-500">not booked</span>}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <p className="mt-3 text-[11px] text-slate-500">Red &quot;Last test&quot; = no test for 7+ days. Click a name for the full analysis.</p>
      </div>
    </div>
  );
}
